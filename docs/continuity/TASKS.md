...
---
## [2026-08-14] Checkpoint: Extraer `GrammarStrategiesPanel` de `GrammarTab` y seguir descomponiendo componentes gigantes
**Rama:** `main`. **Motivo:** continuar P1 sin reabrir `useProjectShell`, avanzando por secciones
pequeñas y seguras de los componentes grandes. En este paso se extrajo el panel de estrategias
de `GrammarTab` a un componente dedicado para reducir la superficie de uno de los 4 componentes
gigantes.
**Cambios:**
- `src/components/GrammarStrategiesPanel.tsx` (NUEVO): componente dedicado para el panel de
  estrategias morfosintácticas con formularios de nombre, tipo, roles, categorías, posición,
  forma y notas.
- `src/components/GrammarTab.tsx`: se reemplazó el bloque inline de `renderStrategies` por el
  componente `<GrammarStrategiesPanel ... />`, manteniendo la lógica de estado y actualización
  del manifiesto en el componente padre.
- Validaciones: `npm run typecheck` 0 errores, `npm run lint` OK, `npm run build` OK,
  `npx vitest run` 122 passed.
- Próximo bloque ejecutable: continuar extrayendo subcomponentes de `GrammarTab`,
...
---
## [2026-08-14] Checkpoint: Extraer `GrammarMorphologyPanel` de `GrammarTab` y seguir descomponiendo componentes gigantes
**Rama:** `main`. **Motivo:** continuar P1 sin reabrir `useProjectShell`, avanzando por secciones
pequeñas y seguras de los componentes grandes. En este paso se extrajo el panel de morfología
de `GrammarTab` a un componente dedicado para reducir la superficie de uno de los 4 componentes
gigantes.
**Cambios:**
- `src/components/GrammarMorphologyPanel.tsx` (NUEVO): componente dedicado para el panel de
  morfología con afijos, excepciones y reglas.
- `src/components/GrammarTab.tsx`: se reemplazó el bloque inline de `renderMorphology` por el
  componente `<GrammarMorphologyPanel ... />`, manteniendo la lógica de estado y actualización
  del manifiesto en el componente padre.
- Validaciones: `npm run typecheck` 0 errores, `npm run lint` OK, `npm run build` OK,
  `npx vitest run` 122 passed.
- Próximo bloque ejecutable: continuar extrayendo subcomponentes de `GrammarTab`,
...
gigantes.
**Cambios:**
- `src/components/GrammarTabHeader.tsx` (NUEVO): componente dedicado para el header dinámico
  con título por módulo y botones de Asistente/Importar/Guardar/Exportar.
- `src/components/GrammarTab.tsx`: se reemplazó el bloque inline del header por el
  componente `<GrammarTabHeader ... />`, manteniendo el estado y las acciones en el
  componente padre. Se limpiaron imports de íconos que ya no se usan directamente en
  `GrammarTab`.
- Validaciones: `npm run typecheck` 0 errores, `npm run lint` OK, `npm run build` OK,
  `npx vitest run` 122 passed.
- Próximo bloque ejecutable: continuar extrayendo subcomponentes de `GrammarTab`,
...
de `GrammarTab` a un componente dedicado para reducir la superficie de uno de los 4
componentes gigantes.
**Cambios:**
- `src/components/GrammarSyntaxPanel.tsx` (NUEVO): componente dedicado para el panel de
  sintaxis con tabs AST/Preview, editor de árbol editable y regeneración desde gramática.
- `src/components/GrammarTab.tsx`: se reemplazó el bloque inline de `renderSyntax` por el
  componente `<GrammarSyntaxPanel ... />`, manteniendo la lógica de estado y actualización
  del manifiesto en el componente padre. Se ajustaron tipos de props para alinear `preview`
  y `onUpdatePreview` con la forma extendida usada por `GrammarTab`.
- Validaciones: `npm run typecheck` 0 errores, `npm run lint` OK, `npm run build` OK,
  `npx vitest run` 122 passed.
- Próximo bloque ejecutable: continuar extrayendo subcomponentes de `GrammarTab`,
  `SyntaxCanvas` o `EntryEditor`.
...
---
## [2026-08-14] Checkpoint: Extraer `GrammarRolesPanel` de `GrammarTab` y seguir descomponiendo componentes gigantes
**Rama:** `main`. **Motivo:** continuar P1 sin reabrir `useProjectShell`, avanzando por secciones
pequeñas y seguras de los componentes grandes. En este paso se extrajo el panel de roles
sintácticos de `GrammarTab` a un componente dedicado para reducir la superficie de uno de los 4
componentes gigantes.
**Cambios:**
- `src/components/GrammarRolesPanel.tsx` (NUEVO): componente dedicado para el panel de roles
  sintácticos con formularios de nombre, descripción y eliminación.
- `src/components/GrammarTab.tsx`: se reemplazó el bloque inline de `renderRoles` por el
  componente `<GrammarRolesPanel ... />`, manteniendo la lógica de estado y actualización
  del manifiesto en el componente padre.
- Validaciones: `npm run typecheck` 0 errores, `npm run lint` OK, `npm run build` OK,
  `npx vitest run` 122 passed.
- Próximo bloque ejecutable: continuar extrayendo subcomponentes de `GrammarTab`,
