...
const normalizeCategory = (category: string = '') => category.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();

const getEntryLabel = (entry?: LexiconEntry, fallback = '[sin entrada]') => {
...
const getEntryForm = (entry?: LexiconEntry, fallback = '...') => entry?.Léxema?.[0]?.replace(/^-|-$/g, '') || entry?.Raíz || fallback;

const getDefaultPreviewEntries = (lexicon: LexiconEntry[], preview?: GrammarManifest['preview']) => {
    const byId = (id?: string) => id ? lexicon.find(entry => entry.ID === id) : undefined;
    const findByCategory = (...terms: string[]) => lexicon.find(entry => {
    ...
    return {
        ...
        object: byId(preview?.objectEntryId) || lexicon.find(entry => entry.ID !== (byId(preview?.subjectEntryId)?.ID) && normalizeCategory(entry.Categoría).includes('sustantivo')) || lexicon[2] || lexicon[0],
    };
};

const buildPreviewSentence = (manifest: GrammarManifest, lexicon: LexiconEntry[]) => {
    ...
    const applyAffixes = (word: string, category: string) => affixes.reduce((current, affix) => {
        const applies = affix.appliesTo.length === 0 || affix.appliesTo.some(target => normalizeCategory(category).includes(normalizeCategory(target)));
...
const isMeaningfulTypology = (t?: GrammarManifest['typology']): boolean =>
    !!t &&
        ...
        <div className="space-y-4">
            <div className="bg-background rounded-lg p-6 border border-border-dark space-y-4">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <button onClick={addAffix} className="text-sm font-bold text-primary bg-primary/10 hover:bg-primary/20 px-3 py-1.5 rounded-lg border border-primary/30 transition-colors">
                        + Añadir Afijo
                    </button>
                </div>

                {editedManifest.ui?.showTone ? (
                ...
                <label className="flex items-center gap-3 bg-surface rounded-lg p-3 border border-border-dark text-sm text-text-primary">
                    <input
                        type="checkbox"
                        checked={editedManifest.preview?.useLexiconAffixes || false}
                        onChange={(e) => updatePreview({ useLexiconAffixes: e.target.checked })}
                ...
                <div className="space-y-3">
                    {(editedManifest.affixInventory || []).map(affix => (
                        <div key={affix.id} className="grid grid-cols-[auto_1fr_1fr_1fr_1fr_auto] gap-3 items-center bg-surface rounded-lg p-3 border border-border-dark">
                            <input
                                ...
                                onChange={(e) => updateAffix(affix.id, { enabled: e.target.checked })}
                                className="accent-primary"
                                aria-label="Activar afijo"
                            />
                            <input
                                value={affix.form}
                                onChange={(e) => updateAffix(affix.id, { form: e.target.value })}
                            ...
                            <select
                                value={affix.type}
                                onChange={(e) => updateAffix(affix.id, { type: e.target.value as GrammarAffix['type'] })}
                                className="bg-background border border-border-dark rounded px-3 py-2 text-white text-sm focus:border-primary focus:outline-none"
                            >
                            ...
                            <input
                                value={affix.meaning}
                                onChange={(e) => updateAffix(affix.id, { meaning: e.target.value })}
                                className="bg-background border border-border-dark rounded px-3 py-2 text-white text-sm focus:border-primary focus:outline-none"
                                placeholder="plural, pasado..."
                            />
                            <input
                                value={affix.appliesTo.join(', ')}
                                onChange={(e) => updateAffix(affix.id, { appliesTo: e.target.value.split(',').map(s => s.trim()).filter(Boolean) })}
                                className="bg-background border border-border-dark rounded px-3 py-2 text-white text-sm focus:border-primary focus:outline-none"
                                placeholder="sustantivo, verbo"
                            />
                            <button onClick={() => removeAffix(affix.id)} className="p-2 text-text-secondary hover:text-red-400 transition-colors" aria-label="Eliminar afijo">
                                <TrashIcon className="w-4 h-4" />
                            </button>
                        </div>
                    ))}
                    {(editedManifest.affixInventory || []).length === 0 && (
                ...
                {editedManifest.preview?.useLexiconAffixes && lexiconAffixes.length > 0 && (
            ...
            <RuleEditor manifest={editedManifest} onChange={updateManifest} />
        </div>
    );
